...
PosteriorHandler::PosteriorHandler(uint32_t history_length, uint8_t trigger_threshold_single,
  ...
  // would be better to do this in the init list but let's stick to the rules
  this->avgs_ = std::vector<MovingAvg>(category_count, MovingAvg(history_length));
  this->supression_end_ = std::vector<uint32_t>(category_count);
...
esp_err_t PosteriorHandler::Handle(uint8_t* new_posteriors, uint32_t time_ms,
  ...
  {
    avgs_[i].emplace(new_posteriors[i]);
  }
  ...
  *top_category_index = 0;
  ...
  for (size_t i = 0; i < posterior_category_count_; i++)
  {
    uint32_t cur_sum = avgs_[i].get_sum();
    if (cur_sum > max_sum)
    {
      max_sum = cur_sum;
  ...
  // 3. Trigger
  *trigger = max_sum >= posterior_trigger_threshold_;

  // 4. Supression
  if (*trigger)
  {
    if (time_ms > supression_end_[*top_category_index])
      supression_end_[*top_category_index] = time_ms + posterior_suppression_ms_;
    else
      *trigger = 0;
  }
